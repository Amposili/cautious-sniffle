...
def api_ping():
    ...
    with _watchdog_lock:
        ...
        _watchdog_triggered = False   # возобновление связи → снять флаг
...
# ═══════════════════════════════════════════════════════════════════════════════
# DURATION CACHE  п.4.2 ТЗ: длительность определяется один раз при загрузке,
# хранится в JSON-файле. /api/tracks НЕ запускает ffprobe/mpg123.
# ═══════════════════════════════════════════════════════════════════════════════
_dur_cache = {}
...
def get_cached_duration(filename):
    """Возвращает длительность из кэша. Если нет — вычисляет и кэширует."""
    with _dur_lock:
...
def all_off():
    ...
    # Не блокируем поток — снимаем ШИМ серво огня асинхронно
    threading.Timer(0.3, lambda: servo_fire_pwm.ChangeDutyCycle(0)).start()
...
def apply_state(state):
    try:
        ...
        # Принимаем оба формата для совместимости со старыми сценариями
        if 'servo5' in state:
...
def get_tracks():
    ...
    for f in sorted(os.listdir(MUSIC_DIR)):
        ...
        item = {'name': f, 'has_scenario': has_sc, 'duration': get_cached_duration(f)}
...
def upload(track_type):
    ...
    # п.4.2: вычисляем длительность один раз при загрузке и кэшируем
...
# ═══════════════════════════════════════════════════════════════════════════════
# BATCH STATE — принимает все каналы одним запросом (решение HTTP Flood п.2.1)
# Фронтенд дросселирует отправку до 20-22 запросов/сек и упаковывает все
# изменившиеся каналы в один JSON. Поддерживает оба формата ключей серво.
# ═══════════════════════════════════════════════════════════════════════════════
